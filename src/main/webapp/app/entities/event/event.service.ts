...
export class EventService {
    ...
    protected convertDateFromClient(event: IEvent): IEvent {
        const copy: IEvent = Object.assign({}, event, {
            startTime: event.startTime != null && event.startTime.isValid() ? event.startTime.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((event: IEvent) => {
                event.startTime = event.startTime != null ? moment(event.startTime) : null;
            });
        }
        return res;
    }
